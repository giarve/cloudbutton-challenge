...
class Tweet(snscrape_base.Item):
	url: str
	...
	username = snscrape_base._DeprecatedProperty('username', lambda self: self.user.username, 'user.username')
	outlinksss = snscrape_base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks), 'outlinks')
	tcooutlinksss = snscrape_base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(self.tcooutlinks), 'tcooutlinks')
...
@dataclasses.dataclass
class User(snscrape_base.Entity):
...
class TwitterOldDesignScraper(snscrape_base.Scraper):
...
class TwitterAPIScraper(snscrape_base.Scraper):
	...
	def _ensure_guest_token(self, url = None):
		...
		if self._guestToken:
			...
			return
		raise snscrape_base.ScraperException('Unable to find guest token')
	...
	def _get_api_data(self, endpoint, params):
		...
		try:
			...
			raise snscrape_base.ScraperException('Received invalid JSON from Twitter') from e
		return obj
			...
			for entry in entries:
				if entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith('tweet-'):
					if 'tweet' in entry['content']['item']['content']:
						...
						raise snscrape_base.ScraperException(f'Unable to handle entry {entry["entryId"]!r}')
